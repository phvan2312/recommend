...
def main(test_cold_user_path, test_cold_item_path, test_warm_path, train_path, u_preference_path, v_preference_path,
    ...
    u_content_matrix = np.fromfile(file=u_content_path,dtype=np.float32).reshape(n_users,200)
    v_content_matrix = np.fromfile(file=v_content_path,dtype=np.float32).reshape(n_items,200)
    ...
    # model
    model = NN(latent_dim=200,user_feature_dim=200,item_feature_dim=200,out_dim=200,default_lr=learning_rate, k=topk,do_batch_norm=True)
