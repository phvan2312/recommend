...
class EvalBatchSample:
    def __init__(self, user_ids, item_ids, ratings, batch_size, **kargs):
        self.user_ids = user_ids.astype(np.int32)
        self.item_ids = item_ids.astype(np.int32)
        self.ratings  = ratings.astype(np.float32)

        if kargs.get('is_cold_user',False):
            #print ('-- is cold start user')
            self.is_cold_user = True
    ...
    def get_batch(self, batch_id ,u_pref, v_pref, u_cont, v_cont, u_bias, v_bias):
        batch_start, batch_end = self.eval_batchs[batch_id]

        feed_dict = {
            'u_pref': u_pref[self.mapping_user_ids.keys()][batch_start:batch_end],
            'v_pref': v_pref[self.mapping_item_ids.keys()],
            'u_cont': u_cont[self.mapping_user_ids.keys()][batch_start:batch_end],
            'v_cont': v_cont[self.mapping_item_ids.keys()],
            'u_bias': u_bias[self.mapping_user_ids.keys()][batch_start:batch_end],
            'v_bias': v_bias[self.mapping_item_ids.keys()]
        }

        if hasattr(self, 'is_cold_user') and self.is_cold_user:
            #print ('modify input for cold-start')
            feed_dict['u_pref'] = np.zeros_like(feed_dict['u_pref'])
            feed_dict['u_bias'] = np.zeros_like(feed_dict['u_bias'])
